...
class PulseGradWD(Optimizer):
    r"""PulseGrad with weight decay (H4).

    ONLY CHANGE: weight_decay=1e-2 (was 0 in baseline)

    Justification: Lion uses 1e-2, Sophia uses 1e-1. Both beat baseline.
    Weight decay helps generalization. This is the minimal change to test
    if regularization closes the 0.0067% gap.
    ...
    """
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), gamma=0.3, eps=1e-8, weight_decay=1e-2):
        if lr < 0.0:
        ...
        super(PulseGradWD, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(PulseGradWD, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                grad = p.grad
                if grad.is_sparse:
                    raise RuntimeError('PulseGradWD does not support sparse gradients')
                ...
                # ONLY DIFFERENCE: weight_decay now defaults to 1e-2
                if weight_decay != 0:

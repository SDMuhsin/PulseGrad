...
class PulseGradEnhanced(Optimizer):
    r"""Enhanced PulseGrad with scaled friction coefficient (H3).

    CHANGE: dfc = 1/(1+exp(-alpha*diff)) with alpha=10
    This provides better sensitivity in the useful diff range (0.01-0.1).
    ...
    Arguments:
        ...
        alpha (float, optional): friction coefficient scale (default: 10.0).
    """
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), gamma=0.3, eps=1e-8, weight_decay=0, alpha=10.0):
        if lr < 0.0:
        ...
        super(PulseGradEnhanced, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(PulseGradEnhanced, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                grad = p.grad
                if grad.is_sparse:
                    raise RuntimeError('PulseGradEnhanced does not support sparse gradients')
                ...
                # Compute ENHANCED friction coefficient (H3: scaled sigmoid)
                # CHANGE: multiply diff by alpha before sigmoid
                dfc = 1.0 / (1.0 + torch.exp(-alpha * diff))
